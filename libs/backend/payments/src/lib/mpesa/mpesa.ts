...
import { HttpService } from './mpesa-http.service';
import { routes } from './mpesa-routes';
import {
...
export class Mpesa {
  ...
  constructor(
    { clientKey, clientSecret }: CredentialsInterface,
   ...
   *  
   M-Pesa completes or the transaction depending on the validation response it receives from the 3rd party system. A confirmation request of the transaction is then sent by M-Pesa through the confirmation URL back to the 3rd party which then should respond with a success acknowledging the confirmation.
   *
